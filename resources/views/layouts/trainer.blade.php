...
<head>
    ...
    <title>{{ config('app.name', 'Gym Trainer') }} — @yield('title', 'Dashboard')</title>
    ...
    <style>
        ...
        .stat-card { background: #ffffff; border-radius: 1.25rem; border: 1px solid #f3f4f6; text-align: center; padding: 1.75rem 1.25rem; }
        ...
        .btn-outline { border: 1.5px solid #e5e7eb; color: #6b7280; padding: 0.6rem 1.25rem; border-radius: 0.75rem; font-weight: 600; font-size: 0.8rem; transition: all 0.2s; display: inline-block; }
        ...
        <div class="sidebar w-72 flex-shrink-0 flex flex-col">
            <div class="p-8">
                <div class="flex items-center space-x-3 mb-2">
                    ...
                    <span class="text-xl font-extrabold tracking-tight text-gray-900">
                        ...
                        @else
                            Gym<span class="text-purple-600">Trainer</span>
                        @endif
                    </span>
                </div>
                <p class="text-[10px] text-gray-400 font-bold uppercase tracking-[0.2em] ml-1">Trainer Portal</p>
            </div>

            <!-- Trainer mini-profile in sidebar -->
            <div class="mx-6 mb-4 p-4 bg-gradient-to-br from-purple-50 to-indigo-50 rounded-2xl border border-purple-100">
                <div class="flex items-center space-x-3">
                    <div class="w-10 h-10 bg-purple-600 text-white flex items-center justify-center rounded-xl font-bold text-sm shadow-md shadow-purple-200">
                        {{ strtoupper(substr(auth()->user()->name, 0, 2)) }}
                    </div>
                    <div>
                        <p class="text-sm font-bold text-gray-900 leading-tight">{{ auth()->user()->name }}</p>
                        <p class="text-[10px] text-purple-600 font-bold uppercase tracking-widest">Trainer</p>
                    </div>
                </div>
            </div>

            <nav class="flex-1 px-6 space-y-1.5 overflow-y-auto mt-2">
                <a href="{{ route('trainer.dashboard') }}" id="nav-overview"
                   class="nav-link flex items-center px-4 py-3 text-gray-500 {{ request()->routeIs('trainer.dashboard') ? 'active' : '' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"/></svg>
                    Overview
                </a>

                <a href="{{ route('trainer.classes') }}" id="nav-classes"
                   class="nav-link flex items-center px-4 py-3 text-gray-500 {{ request()->routeIs('trainer.classes') ? 'active' : '' }}">
                    <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                    My Classes
                </a>
                <a href="{{ route('profile.show') }}" id="nav-profile"
                   class="nav-link flex items-center px-4 py-3 text-gray-500 {{ request()->routeIs('profile.show') ? 'active' : '' }}">
            ...
            <div class="p-8 border-t border-gray-50">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" id="trainer-logout-btn"
                            class="flex items-center w-full px-4 py-3 text-sm font-semibold text-red-500 hover:bg-red-50 rounded-xl transition-all">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <header class="bg-white/80 backdrop-blur-md border-b border-gray-100 py-5 px-10 flex justify-between items-center z-10">
                <div>
                    ...
                    <p class="text-xs text-gray-400 font-medium">@yield('subtitle', 'Manage your schedule and classes')</p>
                </div>
                <div class="flex items-center space-x-4 bg-gray-50 py-1.5 pl-4 pr-1.5 rounded-full border border-gray-100">
                    <div class="text-right">
                        ...
                        <p class="text-[9px] text-purple-600 font-extrabold uppercase tracking-widest">Premium Trainer</p>
                    </div>
